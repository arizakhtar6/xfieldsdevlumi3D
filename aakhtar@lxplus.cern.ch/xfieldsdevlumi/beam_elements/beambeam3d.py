                ...
                if self.config_for_update.pipeline_manager.is_ready_to_send(self.config_for_update.element_name,
                    ...
                    if self.flag_numerical_luminosity == 1:
                        ...
                        fmap = TriLinearInterpolatedFieldMap(
                            ...
                            z_range=z_lim, nz=3,
                            solver='FFTSolver3D')
                        fmap.update_from_particles(x_p=particles.x, y_p=particles.y, z_p=np.zeros(self.n_macroparticles),
                        ...
                        self.dx = fmap.dx
                        self.dy = fmap.dy
                        self.lumigrid_my_beam = fmap.rho[:,:,1]
                        print(f"lumigrid_my_beam size: {np.shape(self.lumigrid_my_beam)}")
                        if self.update_lumigrid_sum ==1:  
                            self.lumigrid_sum = self.lumigrid_sum+self.lumigrid_my_beam.flatten()
                        exchange_buffer = self._buffer.context.nplike_lib.hstack([self.moments, self.lumigrid_my_beam.flatten()]) #flattened to 1D
                        print(f"exchange_buffer size: {np.shape(exchange_buffer)}")
