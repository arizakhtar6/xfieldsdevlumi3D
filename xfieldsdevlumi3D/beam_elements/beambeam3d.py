...
class NumLumiTable(xo.HybridClass):
    _xofields = {
        '_index': xt.RecordIndex,
        'at_element': xo.Int64[:],
        'at_turn': xo.Int64[:],
        'particle_id': xo.Int64[:],
        'numerical_luminosity': xo.Float64[:],
    }
...
class BeamBeamBiGaussian3D(xt.BeamElement):
    ...
    _xofields = {
        ...
        'slices_other_beam_sqrtSigma_55_beamstrahlung': xo.Float64[:],
         ...
         #numerical luminosity
         'flag_numerical_luminosity': xo.Int64,
         'n_lumigrid_cells': xo.Int64,
         'sig_lumigrid_cells': xo.Float64,
         'range_lumigrid_cells': xo.Float64,
         'n_macroparticles': xo.Int64,
         'nTurn': xo.Int64,
         'update_grid_sum': xo.Int64,
    }
    ...
    def __init__(self,
                    ...
                    n_lumigrid_cells = 0,
                    sig_lumigrid_cells = 0,
                    range_lumigrid_cells = 0,
                    n_macroparticles = 0,
                    nTurn = 0,
        ...
        self._allocate_xobject(n_slices, **kwargs)  # NEW
        if config_for_update is not None:
...
#                    int(n_lumigrid_cells)**2*n_slices, dtype=float)  # NEW, nxn flattened grid, int but concat will make it float anyway
        ...
        self._init_numerical_luminosity(flag_numerical_luminosity, n_lumigrid_cells, sig_lumigrid_cells, range_lumigrid_cells, n_macroparticles, nTurn, frev, update_lumigrid_sum)
    ...
    def _init_numerical_luminosity(self, flag_numerical_luminosity, n_lumigrid_cells, sig_lumigrid_cells, range_lumigrid_cells, n_macroparticles, nTurn, frev, update_lumigrid_sum):
        self.flag_numerical_luminosity = flag_numerical_luminosity
        self.n_lumigrid_cells = n_lumigrid_cells
        self.sig_lumigrid_cells = sig_lumigrid_cells
        self.range_lumigrid_cells = range_lumigrid_cells
        self.n_macroparticles = n_macroparticles
        self.nTurn = nTurn
        self.frev = frev
        self.numlumitable = NumLumiTable(
            _context=self._context,  # Assuming you have a context set up for GPU or other environments
            at_element=xo.Int64[:](nTurn),
            at_turn=xo.Int64[:](nTurn),
            particle_id=xo.Int64[:](nTurn),
            numerical_luminosity=xo.Float64[:](nTurn))
        self.lumigrid_sum = np.zeros(int(n_lumigrid_cells)**2 * (2*self.config_for_update.slicer.num_slices -1), dtype=float)
        self.update_lumigrid_sum = update_lumigrid_sum
    ...
    def update_from_received_lumigrid(self):  # NEW
        ...
        print(np.sum(self.lumigrid_other_beam))
        ...
        while True:
            ...
            if (self.config_for_update._do_update and (not self.config_for_update.quasistrongstrong
                ...
                if self.config_for_update.pipeline_manager.is_ready_to_send(self.config_for_update.element_name,
                    ...
                    #print(start)
                    #print(min(self.config_for_update.slicer.num_slices - 1, self.config_for_update._i_step) + 1)
                    ...
                    for i in range(max(0, (self.config_for_update._i_step - (self.config_for_update.slicer.num_slices - 1))), min(self.config_for_update.slicer.num_slices - 1, self.config_for_update._i_step) + 1):
                        ...
                        #self.lumigrid_my_beam = np.zeros((nx , ny, j+start))
                        #self.lumigrid_visualisation = np.zeros((nx, ny, 2*self.config_for_update.slicer.num_slices - 1))
                        
                        lumi_index = i - start # apply correction to the index to get the correct slice number
                        # Loop over slices
                        self.config_for_update.slicer.assign_slices(particles)  # in this the bin edges are fixed with TempSlicer
                        particles_in_slice = particles.filter(particles.slice==i)
                        
                        # compute interacting other beam slice ID, this is for the other slice, particles_in_slice gives the slice index for my beam
                        ...
                        # Should give 2*num_slices - 1 CPs 
                        S = 0.5 * (particles_in_slice.zeta - self.slices_other_beam_zeta_center_star[self.config_for_update._i_step - i])
                        x_bar_star = particles_in_slice.x + particles_in_slice.px * S - self.slices_other_beam_x_center_star[self.config_for_update._i_step - i]
                        y_bar_star = particles_in_slice.y + particles_in_slice.py * S - self.slices_other_beam_y_center_star[self.config_for_update._i_step - i]
                        # print("x_bar_star:", x_bar_star)
                        # print("y_bar_star:", y_bar_star)
                        # print("z_p:", np.zeros(self.n_macroparticles))
                        # print(x_bar_star.shape, y_bar_star.shape)
                        # print("ncharges_p:", np.ones(self.n_macroparticles))
                        if self.flag_numerical_luminosity == 1: 
                            sigma = self.sig_lumigrid_cells
                            my_range = self.range_lumigrid_cells/2
                            x_lim = (-my_range*np.std(particles.x), my_range*np.std(particles.x))
                            y_lim = (-my_range*np.std(particles.y), my_range*np.std(particles.y))
                            z_lim = (-my_range*np.std(particles.zeta), my_range*np.std(particles.zeta))
                            fmap = TriLinearInterpolatedFieldMap(
                                x_range=x_lim, nx=self.n_lumigrid_cells,
                                y_range=y_lim, ny=self.n_lumigrid_cells,
                                z_range=z_lim, nz=3,
                                solver='FFTSolver3D')
                            fmap.update_from_particles(x_p=x_bar_star, y_p=y_bar_star, z_p=np.zeros(len(x_bar_star)),
                            ncharges_p=np.ones(len(x_bar_star)), q0_coulomb=1., update_phi = False)   # call C function to compute my beams lumigrid here, e.g. 2 slices, 3 by 3 grid for each slice
                            # print("x_range:", x_lim)
                            # print("y_range:", y_lim)
                            # print("z_range:", z_lim)
                            # print("nx:", self.n_lumigrid_cells)
                            # print("ny:", self.n_lumigrid_cells)
                            # print("nz:", 3) 
                            
                            # print("Particle x range:", np.min(x_bar_star), np.max(x_bar_star))
                            # print("Grid x range:", x_lim)
                            # print("Particle y range:", np.min(y_bar_star), np.max(y_bar_star))
                            # print("Grid y range:", y_lim)
                            # print("Particle z range:", np.min(np.zeros(self.n_macroparticles)), np.max(np.zeros(self.n_macroparticles)))
                            # print("Grid z range:", z_lim)
                            self.dx = fmap.dx
                            self.dy = fmap.dy
                            # print("dz: ", fmap.dz)
                            # print(fmap)
                ...
                if self.config_for_update.pipeline_manager.is_ready_to_receive(self.config_for_update.element_name,
                                    ...
                                    internal_tag=self.config_for_update._i_step):
                        # NEW
                    self.config_for_update.pipeline_manager.receive_message(self.partner_buffer,
                                        self.config_for_update.element_name,
                                    self.config_for_update.partner_particles_name,
                                        particles.name,
                                        internal_tag=self.config_for_update._i_step)
                    ...
                    if self.flag_numerical_luminosity == 1:
                        ...
                        self.partner_moments = self.partner_buffer[:]
                        self.update_from_recieved_moments()
            ...
            if self.flag_numerical_luminosity == 1:
                ...
                for i in range(max(0, (self.config_for_update._i_step - (self.config_for_update.slicer.num_slices - 1))), min(self.config_for_update.slicer.num_slices - 1, self.config_for_update._i_step) + 1):
                    start = max(0, self.config_for_update._i_step - (self.config_for_update.slicer.num_slices - 1))
                    stop = min(self.config_for_update.slicer.num_slices - 1, self.config_for_update._i_step) + 1
                    #print("Start for this computation: ",start)
                    #j = self.config_for_update._i_step + 1 if self.config_for_update._i_step <= self.config_for_update.slicer.num_slices//2 + 1 else 2 * self.config_for_update.slicer.num_slices - 1 - self.config_for_update._i_step # Gives the number of interacting slices for each timestep   
                    print("Sum of the whole my beam: ", np.sum(self.lumigrid_my_beam))
                    print("Sum of the indexed my beam: ", np.sum(self.lumigrid_my_beam[:,:,i-start]))
                    sum_lumigrid_my_beam = np.sum(self.lumigrid_my_beam[:,:,i-start])
                    sum_lumigrid_other_beam = np.sum(self.lumigrid_other_beam)
                    print("Sum of the whole other beam: ", np.sum(self.lumigrid_other_beam))
                    print("Sum of my indexed other beam: ", np.sum(self.lumigrid_other_beam[:,:,start+stop-1-(i)]))
                    ...
                    self.numlumitable.numerical_luminosity[at_turn] += compute_lumi_integral(timestep=self.config_for_update._i_step, 
                        ...
                        lumigrid_other_beam=self.lumigrid_other_beam[:,:,start+stop-1-i],
                        frev = self.frev,
                    ...
                    print(self.config_for_update._i_step)
                    print("Lumigrid my beam indices:", i-start)
                    print("Lumigrid other beam indices:", start+stop-1-i)
...
def compute_lumi_integral(lumigrid_my_beam, lumigrid_other_beam, timestep, frev, beam_intensity, other_beam_intensity, dx, dy):
    sum1 = np.sum(lumigrid_my_beam)
    sum2 = np.sum(lumigrid_other_beam)
    #print(sum1, sum2)
    #print(frev, beam_intensity, other_beam_intensity)
    #print(dx,dy)
    ...
    if sum2 ==0.0:
        ...
        scale2 = 1.0/(sum2*dx*dy)
    print(f"sum1: {sum1}, dx: {dx}, dy: {dy}, scale1: {scale1}")
    print(f"sum2: {sum2}, dx: {dx}, dy: {dy}, scale2: {scale2}")
    h1_scaled = lumigrid_my_beam*scale1
    h2_scaled = lumigrid_other_beam*scale2
    h_multiplied = h1_scaled*h2_scaled
    nx = lumigrid_my_beam.shape[0]
    ny = lumigrid_my_beam.shape[1]
    ...
    return numerical_luminosity # Call C function to compute the integral based on the 2 grids at timestep, grids contain the status at timestep for every slice but only overlapping will interact
...
#     if sum1 == 0 or dx == 0 or dy == 0:
#         scale1 = 0.0
#     else:
#         scale1 = 1.0 / (sum1 * dx * dy)

#     if sum2 == 0 or dx == 0 or dy == 0:
#         scale2 = 0.0
#     else:
#         scale2 = 1.0 / (sum2 * dx * dy)
...
#     nx, ny = lumigrid_my_beam.shape
#     integral = 0.25 * dx * dy * (h_multiplied[0, 0] + h_multiplied[-1, 0] + h_multiplied[0, -1] + h_multiplied[-1, -1])
#     secondPart = np.sum(h_multiplied[1:, 1:])
#     thirdPart = np.sum(h_multiplied[1:, [0, -1]])
#     fourthPart = np.sum(h_multiplied[[0, -1], 1:])
