...
#                    int(n_lumigrid_cells)**2*n_slices, dtype=float)  # NEW, nxn flattened grid, int but concat will make it float anyway
        ...
        self._init_numerical_luminosity(flag_numerical_luminosity, n_lumigrid_cells, sig_lumigrid_cells, range_lumigrid_cells, n_macroparticles, nTurn, update_lumigrid_sum)
    ...
    def _init_numerical_luminosity(self, flag_numerical_luminosity, n_lumigrid_cells, sig_lumigrid_cells, range_lumigrid_cells, n_macroparticles, nTurn, update_lumigrid_sum):
        ...
        self.lumigrid_sum = np.zeros(int(n_lumigrid_cells)**2, dtype=float)
        ...
        while True:
            ...
            if (self.config_for_update._do_update and (not self.config_for_update.quasistrongstrong
                ...
                if self.config_for_update.pipeline_manager.is_ready_to_send(self.config_for_update.element_name,
                    ...
                    j = self.config_for_update._i_step + 1 if self.config_for_update._i_step <= self.config_for_update.slicer.num_slices//2 else self.config_for_update.slicer.num_slices - self.config_for_update._i_step # Gives the number of interacting slices for each timestep   
                    lumi_slice = np.zeros((nx , ny, j))    
                    for i in range(np.max(0, self.config_for_update.slicer.num_slices//2 - self.config_for_update._i_step), np.min(self.config_for_update.slicer.num_slices - 1, self.config_for_update._i_step) + 1):
                        # Loop over slices
                        self.config_for_update.slicer.assign_slices(particles)  # in this the bin edges are fixed with TempSlicer
                        ...
                        S = 0.5 * (particles_in_slice.zeta - self.slices_other_beam_zeta_center_star[self.config_for_update._other_beam_slice_index_for_particles])
                        x_bar_star = particles_in_slice.x + particles_in_slice.px * S - self.slices_other_beam_x_center_star[self.config_for_update._other_beam_slice_index_for_particles] # Do I need to even loop over particles if i am doing it like this?
                        y_bar_star = particles_in_slice.y + particles_in_slice.py * S - self.slices_other_beam_y_center_star[self.config_for_update._other_beam_slice_index_for_particles]
                        #cp_coords.append([x_bar_star, y_bar_star, S])
                        if self.flag_numerical_luminosity == 1: 
                            sigma = self.sig_lumigrid_cells
                            range = self.range_lumigrid_cells/2
                            x_lim = (-range*np.std(particles.x), range*np.std(particles.x))
                            y_lim = (-range*np.std(particles.y), range*np.std(particles.y))
                            z_lim = (-range*np.std(particles.zeta), range*np.std(particles.zeta))
                            fmap = TriLinearInterpolatedFieldMap(
                                x_range=x_lim, nx=self.n_lumigrid_cells,
                                y_range=y_lim, ny=self.n_lumigrid_cells,
                                z_range=z_lim, nz=j,
                                solver='FFTSolver3D')
                            fmap.update_from_particles(x_p=x_bar_star, y_p=y_bar_star, z_p=np.zeros(self.n_macroparticles),
                            ncharges_p=np.ones(self.n_macroparticles), q0_coulomb=1., update_phi = False)   # call C function to compute my beams lumigrid here, e.g. 2 slices, 3 by 3 grid for each slice
                        
                            self.dx = fmap.dx
                            self.dy = fmap.dy
                            
                            
                            lumi_slice = fmap.rho[:,:,1]
                            
                            self.lumigrid_my_beam = np.sum(lumi_slice, axis=2) # sum along z
                            ...
                            self.numlumitable.numerical_luminosity[at_turn] = compute_lumi_integral(timestep=self.config_for_update._i_step, 
                                ...
                                frev = 11245.5,
                                beam_intensity = 0.7825E11, other_beam_intensity = 0.7825E11, dx = self.dx, dy = self.dy)  # NEW, C function to compute integral based on 2 grids at timestep _i_step, grids contain the status at _i_step for every slice but only overlapping will interact
                            
                        
                            if self.update_lumigrid_sum ==1:  
                                self.lumigrid_sum[i_step*nx*ny:(i_step+1)*nx*ny] = self.lumigrid_my_beam.flatten() # sum along z
